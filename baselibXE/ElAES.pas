...
type
  ...
  TKeyBuffer = array[0..255] of byte;
...
function DecryptAESString(Value: String; Key: TKeyBuffer; KeyBit: TKeyBit): String;
function EncryptAESString(Value: String; Key: TKeyBuffer; KeyBit: TKeyBit): String;
...
implementation
uses System.RTLConsts,Soap.EncdDecd,System.ZLib;
...
function EncryptAESString(Value: String; Key: TKeyBuffer;
...
begin
  ...
  try
    ...
    begin
      ...
      Move(Key, AESKey128, Min(SizeOf(AESKey128), Length(Key)));
    ...
    begin
      ...
      Move(Key, AESKey192, Min(SizeOf(AESKey192), Length(Key)));
    ...
    begin
      ...
      Move(Key, AESKey256, Min(SizeOf(AESKey256), Length(Key)));
...
function DecryptAESString(Value: String; Key: TKeyBuffer;
  KeyBit: TKeyBit): String;
var
  SS, DS: TStringStream;
  MS: TMemoryStream;
...
begin
    ...
    try
      ...
      OutStream.Write(Size, SizeOf(Integer));
      ZStream := TCompressionStream.Create(clFastest, mStream);
      try
        ZStream.CopyFrom(InStream, 0);
      finally
        ZStream.Free;
      end;
      OutStream.Position:=SizeOf(Integer);
      OutStream.Write(mStream.Memory^, mStream.Size);
    finally
      mStream.Free;
    end;
end;
...
begin
    ...
    InStream.Position:=0;
    InStream.Read(Size,Sizeof(Integer));
    if Size = 0 then Exit;
     mStream:=TMemoryStream.Create;
     InStream.Position:=Sizeof(Integer);
     mStream.Size:=InStream.Size - Sizeof(Integer);
     InStream.Read(mStream.Memory^, InStream.Size - Sizeof(Integer));
     try
