...
bool __fastcall TfrmClientVisitEdit::VerifyData()
{
	...
	{
        ShowMessage(L"请输入客户编号!");
		return false;
	}
	if(edName->Text.IsEmpty())
	{
        ShowMessage(L"请输入客户名称!");
		return false;
	}
	if(edPhone->Text.IsEmpty())
	{
		ShowMessage(L"请输入客户电话!");
		return false;
	}
	return true;
}
...
//	FSTMessageDlg(_D("确认要删除数据吗？") ,TMsgDlgType::mtConfirmation,mbYesNo,0,msgDialogDeleteProc);
}
//---------------------------------------------------------------------------
