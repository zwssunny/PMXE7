...
// ---------------------------------------------------------------------------
TSendMessageThread::TSendMessageThread(TDSServer *Server, TLoginUserRecord* User, TJSONValue *MsgValue)
{
	FServer = Server;
	AccBookID = User->AccBookID;
	UserID = User->UserID;
	UserName = User->UserName;
	CallBackClientID = User->CallBackClientID;
	FMsgValue=(TJSONValue*)(MsgValue->Clone());
}
// ---------------------------------------------------------------------------
void __fastcall  TSendMessageThread::Execute()
{
  ...
  FServer->NotifyCallback(CallBackClientID,SMsgCallback, FMsgValue,Resp);
}
// ---------------------------------------------------------------------------
...
void __fastcall TBootUserThread::Execute() {
	...
	FreeOnTerminate = true;
	if ((FServer == NULL) || (CallBackClientID == EmptyStr))
		return;
	Tunnel = FServer->GetCallbackTunnel(CallBackClientID);
	if (Tunnel != NULL) {
		...
		FServer->UnregisterChannelCallback(CallBackClientID,SMsgCallback, SecurityToken);
...
void TSendMissMessageThread::SendMissMessage() {
	...
	String SQLText="select tbMissMember.RecMsgID,tbMissMember.RecMsgUserID, tbMessageRecords.*,EmpName as username "
					...
					" where RecMsgRead=0 and RecMsgUserID ='"+UserID + "' order by MsgSendTime";
