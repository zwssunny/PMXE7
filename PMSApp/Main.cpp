...
void __fastcall TMainForm::DeleteGroupItems()
{
	...
	for(int i=0;i<gcount;i++)
	{
	 ...
	 {
		 TfrmMessageForm *msgform=static_cast<TfrmMessageForm*>(curItem->TagObject);
		 if(msgform)
		 {
			  delete msgform;
			  msgform=NULL;
...
void __fastcall TMainForm::tvCompanyDeletion()
{
  ...
  for(int i=0;i<tvCompany->GlobalCount;i++)
  {
	 ...
	 if(ANode->TagObject)
	 {
		 TfrmMessageForm *msgform=static_cast<TfrmMessageForm*>(ANode->TagObject);
		 if(msgform)
		 {
			  delete msgform;
			  msgform=NULL;
