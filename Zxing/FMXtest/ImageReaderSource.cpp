...
inline char ImageReaderSource::convertPixel(char const* pixel_) const {
  ...
  if (comps == 1 || comps == 2) {
	...
	throw IllegalArgumentException("Unexpected image depth");
  }
}
...
Ref<LuminanceSource> ImageReaderSource::create(const unsigned char* in, size_t insize) {

  unsigned char picformat=in[1];
  int width, height;
  int comps = 0;
  zxing::ArrayRef<char> image;
  if (picformat == 0x50) {   //png
	std::vector<unsigned char> out;

    { unsigned w, h;
	  unsigned error = lodepng::decode(out, w, h, in,insize);
      if (error) {
        ostringstream msg;
        msg << "Error while loading '" << lodepng_error_text(error) << "'";
        throw zxing::IllegalArgumentException(msg.str().c_str());
    ...
    image = zxing::ArrayRef<char>(4 * width * height);
    memcpy(&image[0], &out[0], image->size());
  } else /*if (picformat == 0x4A)*/ {    //jpg
	char *buffer = reinterpret_cast<char*>(jpgd::decompress_jpeg_image_from_memory(
		in,insize, &width, &height, &comps, 4));
	image = zxing::ArrayRef<char>(buffer, 4 * width * height);
  }
  if (!image) {
    ostringstream msg;
    msg << "Loading jpg memory failed.";
    throw zxing::IllegalArgumentException(msg.str().c_str());
  }
