...
bool __fastcall TfrmStuffStockInEdit::BeforeUpdateData(int OptType)
{
  ...
  switch(OptType)
  {
    case 0:
	  if(FPrjRight<1)
      {
		ShowMessage(_D("您没有与该仓库关联的项目的读权限"));
        return false;
      }
	  return true;
	case 1:
    ...
    case 3:   //Del
	  if(FPrjRight<2)
	  {
		ShowMessage(_D("您没有该仓库关联的项目的写权限"));
		return false;
      }
      return true;
    default:
...
void __fastcall TfrmStuffStockInEdit::AfterDataSetOperate(TOperateType OptType)
{
  if (IsBalance && OptType == otUpdate)
  {
		...
		{
		   TMessageDialogEventProc msgDialogCloseProc=&DoBalanceGen;
		   ShowMessageBox(L"是否自动生成采购结算单?",msgDialogCloseProc);
		}
		IsBalance = false;
		TMessageDialogEventProc msgDialogCloseProc=&DoGenAdjBill;
		ShowMessageBox(L"是否根据差价自动生成入库调整单，出库调整单?",msgDialogCloseProc);

	}
}
//--------------------------------------------------------------------------
void __fastcall TfrmStuffStockInEdit::DoBalanceGen(System::Uitypes::TModalResult AResult)
{
 if(AResult==mrYes)
 {
	Query->Close();
...
//--------------------------------------------------------------------------
void __fastcall TfrmStuffStockInEdit::DoGenAdjBill(System::Uitypes::TModalResult AResult)
{
 if(AResult==mrYes)
 {
	try
...
//---------------------------------------------------------------------------
void __fastcall TfrmStuffStockInEdit::DoDeleteDetail(System::Uitypes::TModalResult AResult)
{
 if(AResult==mrYes)
 {
  DataSet->DetailFirst();
  ...
  String 	SQLStr = " select StuSpyNtPKID,StuSpyNtFKID_Prj,StuSpyNtNum,StuSpyNtFKID_BuyCont,"
					 ...
					 " where StuSpyNtPKID="+QuotedStr(GetMasterValue("StuSkInFKID_Rep"));
  Query->Close();
  Query->SQL->Text=SQLStr;
  Query->Open();
  if(Query->RecordCount>0)
...
void __fastcall TfrmStuffStockInEdit::SetStuSupplyNote(String StuSupplyNoteID)
{
  ...
  {
   TMessageDialogEventProc msgDialogCloseProc=&DoDeleteDetail;
   ShowMessageBox(L"更改供货单号，将删除所有明细行，确认吗?",msgDialogCloseProc);
  }
...
void __fastcall TfrmStuffStockInEdit::DepotNameButtonClick(TObject *Sender)
{
 if(InStoreFilter==NULL)
 {
  ...
  String FSQL=" select StuDepPKID,StuDepName,StuDepFKID_Prj,StuDepFKID_Cont,PrjAmender=isnull(PrjAmender,''),PrjFKID_StartCyc=isnull(PrjFKID_StartCyc,''),\
