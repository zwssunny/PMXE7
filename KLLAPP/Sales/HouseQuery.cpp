...
{
  if(InforForm==NULL)
   InforForm=new THouseInformationForm(this,ClientBroker,this->FormModuleCode,FDataSet,status);
  return InforForm;
}
//---------------------------------------------------------------------------
...
{
  if(this->RecordCount>0)  //未订状态
  {
	if(1==GetFieldValue("HsgState"))
	{
...
void __fastcall TfrmHouseQuery::HouseViewActionExecute(TObject *Sender)
{
   ...
   if(HouseStringGrid->Selected>-1)
      HouseViewForm->SetCurHouseType(GetFieldValue("EstateName"),GetFieldValue("BdgName"),GetFieldValue("HsgHTRemark"));
   HouseViewForm->Show();
}
//---------------------------------------------------------------------------
