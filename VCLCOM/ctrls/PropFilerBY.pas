...
type
  ...
  TWriteOwnerPropsEventBY = procedure(Writer: TPropWriterBY) of object;
  TReadOwnerPropEventBY = procedure(Reader: TPropReaderBY; PropName: String;
    var Processed: Boolean) of object;

 {TPropWriterBY}

  TPropWriterBY = class(TWriter)
  private
    FCurRootsList: TList;
...
//    FLookupRoot: TComponent;
    FPropPath: String;
    FOnWriteOwnerProps: TWriteOwnerPropsEventBY;
  ...
  public
    ...
    property OnWriteOwnerProps: TWriteOwnerPropsEventBY read FOnWriteOwnerProps write FOnWriteOwnerProps;
  end;

 {TPropReaderBY}

  TPropReaderBY = class(TReader)
  private
    FCanHandleExcepts: Boolean;
    FCollectionList: TList;
    FInterceptorList: TList;
    FPropName: String;
    FOnReadOwnerProp: TReadOwnerPropEventBY;
...
{$ENDIF}
  ...
  public
    ...
    property OnReadOwnerProp: TReadOwnerPropEventBY read FOnReadOwnerProp write FOnReadOwnerProp;
  end;
...
{ TFormStoragePropertyInterceptor }
  ...
  procedure GetComponentChildListBY(ParentComp, Root: TComponent; cl: TStrings; CheckInline: Boolean);
  function FindChildComponent(ParentComp, Root: TComponent; const AName: string; CheckInline: Boolean): TComponent;

var
  IsRaiseReadErrorBY: Boolean = False;

implementation


uses {$IFDEF BY_LIB_6} ConvUtils, RTLConsts {$ELSE} Consts {$ENDIF};
...
begin
  ...
  try
    GetComponentChildListBY(ParentComp, Root, ChildList, CheckInline);
...
constructor TPropWriterBY.Create(Stream: TStream; BufSize: Integer);
begin
...
destructor TPropWriterBY.Destroy;
var
 i: Integer;
begin
...
procedure TPropWriterBY.BuildPropsList(AObject: TObject; sl: TStrings);
var
  PropList: PPropList;
...
procedure TPropWriterBY.WriteOwnerProperties(Owner: TComponent; PropList: TStrings);
var
  i, j, Level: Integer;
  Path: String;
  PPos: Integer;
...
procedure TPropWriterBY.SaveObjectProperyValue(Instance: TObject; Path, FullPath: String);
var
  PropInfo: PPropInfo;
  PropType: PTypeInfo;
  PropName: String;
...
{$IFDEF BY_LIB_6} and not (csSubComponent in TComponent(Value).ComponentStyle) {$ENDIF} then
      begin
...
procedure TPropWriterBY.WritePropName(const PropName: string);
begin
  WriteStr(FPropPath + PropName);
end;

procedure TPropWriterBY.DefineBinaryProperty(const Name: string; ReadData,
...
procedure TPropWriterBY.DefineProperty(const Name: string;
...
procedure TPropWriterBY.WriteCollection(Value: TCollection);
var
  I: Integer;
  OldAncestor: TPersistent;
begin
...
procedure TPropWriterBY.WriteAllProperties(Instance: TObject);
var
  I: Integer;
  sl: TStringList;
begin
...
procedure TPropWriterBY.DefineObjectProperties(Instance: TObject);
var
  i: Integer;
begin
  for i := 0 to FDefnPropList.Count-1 do
...
constructor TPropReaderBY.Create(Stream: TStream; BufSize: Integer);
begin
  inherited Create(Stream, BufSize);
  FCollectionList := TList.Create;
end;

destructor TPropReaderBY.Destroy;
begin
  FCollectionList.Free;
  inherited Destroy;
end;

procedure TPropReaderBY.ReadComponent(Component: TComponent);
var
  I: Integer;
  Flags: TFilerFlags;
  CompName: String;
...
procedure TPropReaderBY.ReadCollection(Collection: TCollection);
var
  Item: TPersistent;
begin
...
procedure TPropReaderBY.ReadProperty(AInstance: TPersistent);
var
  I, J, L: Integer;
...
function TPropReaderBY.Error(const Message: string): Boolean;
begin
  Result := inherited Error(Message);
  if not IsRaiseReadErrorBY then
    Result := True;
end;

procedure TPropReaderBY.ReadPropValue(Instance: TPersistent; PropInfo: Pointer);
const
...
procedure TPropReaderBY.DefineProperty(const Name: string;
  ReadData: TReaderProc; WriteData: TWriterProc; HasData: Boolean);
begin
...
procedure TPropReaderBY.DefineBinaryProperty(const Name: string;
...
procedure TPropReaderBY.ReadOwnerProperties(Component: TComponent);
begin
  ReadSignature;
  Root := Component;
  Owner := Root;
...
function TPropReaderBY.ReadSet(SetType: Pointer): Integer;
var
  EnumType: PTypeInfo;
  EnumName: string;
begin
...
procedure TPropReaderBY.SkipSetBody;
begin
...
procedure TPropReaderBY.PropertyError;
begin
  SkipValue;
  PropertyNotFound;
end;

procedure TPropReaderBY.SkipValue;
...
procedure TPropReaderBY.SkipProperty;
begin
