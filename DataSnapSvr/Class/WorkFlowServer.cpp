...
void __fastcall TWorkFlowServer::InitWorkFlowStudio()
{
   ...
   //	TLoginUserRecord *curUser = GetCurLoginUser();
	WorkflowStudio->UserManager->LoggedUserId   = curUser->UserID;
	WorkflowStudio->UserManager->LoggedUserCode = curUser->UserCode;
	...
	for (int i=0; i < CallbackIDList->Count; i++)
	{
		...
		if(retNotify)  //只有有一个成功则退出循环
		{
		   if(RetJson!=NULL)
		   {
			   ...
			   if(retJSON->Count>1)
			   {
				 if(retJSON->GetValue("result")->Value()=="true")
				   FFlowID= retJSON->GetValue("returnvalue")->Value();
			   }
			   RetJson=NULL;
		   }
		   break;
	...
	for (int i = 0; i < CallbackIDList->Count; i++)
	{
		...
		if(retNotify) //只要成功则返回
		{
		  if(RetJson!=NULL)
		  {
			   ...
			   {
				if(retJSON->GetValue("result")->Value()=="true")
				   retSucce=retJSON->GetValue("returnvalue")->Value()=="true";
			   }
			   RetJson=NULL;
		   }
		   break;  //只有有一个成功则退出循环
...
TStream* TWorkFlowServer::GetTaskListDS(String AUserID, int ACompletedState, String AOtherCondition)
{
	...
	for(int i=0;i<m_Query->FieldCount;i++)
	{
		...
		FMasterDataSet->FieldDefs->Add("FlowMnDtlReceiverName",ftString,50,false);//增加接收者名称
		FMasterDataSet->FieldDefs->Add("FlowMnDtlOperaterName",ftString,50,false);//增加操作者名称
		FMasterDataSet->FieldDefs->Add("FlowMnCreatorName",ftString,50,false);//增加创建者名称
	FMasterDataSet->CreateDataSet();
	m_Query->Close();
	for(int i=0;i<FTaskList->Count;i++)
	{
		...
		if(FTempList!=NULL)
		{
			...
			TWorkflowUser *FUser = WorkflowStudio->UserManager->Users->FindById(Creator);
			if(FUser)
