...
String TTaskShowCol::GetData(TPrjObject&AObj)
{
    switch(DataIndex)
	{
		...
		case 9://计划工程量（进度）			在进度截止日期的计划数由任务日历汇总得到
			return Format("%.2f",ARRAYOFCONST(((double)AObj.WorkLoad)));
		case 10://实际工程量（进度）		汇总	实际工程量只能小于等于计划工程量,当任务完工时二者完全相等
...
String TTaskShowCol::GetTitle()
{
   switch(DataIndex)
   {
	   ...
	   case 8://计划开始时间
		   return "计划开始";
	   case 9://计划进度			在进度截止日期的计划数由任务日历汇总得到
		   return "计划进度";
	   case 10://实际进度		汇总	实际工程量只能小于等于计划工程量,当任务完工时二者完全相等
		   return "实际进度";
	   case 11://进度评估值		汇总	这里会生成百分比
		   return "进度评估值";
	   case 12://计划完成时间
