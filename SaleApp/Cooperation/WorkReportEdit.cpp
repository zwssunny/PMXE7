...
void __fastcall TfrmWorkReportEdit::GetDataFromDataSet(int Section)
{
	if(Section == 1)
	{
		...
		if(GetMasterValue("WorkReportCheckState") == 1)
			seWorkReportState->Text = L"已审核";
		else if(GetMasterValue("WorkReportState") == 1)
			seWorkReportState->Text = L"已提交";
		else
			seWorkReportState->Text = L"草稿";
...
//---------------------------------------------------------------------------
TForm * __fastcall TfrmWorkReportEdit::NewItemEditForm(TZClientDataSet *m_DataSet, int status)//创建并返回明细窗体
{
...
void __fastcall TfrmWorkReportEdit::edWorkflowTitleEnter(TObject *Sender)
{
	if(edWorkflowTitle->Text.IsEmpty())
   {
	String ReportStr="";
	 ...
	 {
		case 0:
		  ReportStr=L"工作周报,"+spWorkReportYear->Text+L"年第"+spWorkReportWeek->Text+L"周";
		  break;
		case 1:
		  ReportStr=L"工作月报,"+spWorkReportYear->Text+L"年第"+spWorkReportWeek->Text+L"月";
		  break;
		case 2:
		  ReportStr=L"工作季报,"+spWorkReportYear->Text+L"年第"+spWorkReportWeek->Text+L"季";
		  break;
		case 3:
		  ReportStr=L"工作年报,"+spWorkReportYear->Text+L"年度";
		  break;
		case 4:
		  ReportStr=L"工作日报,日期"+deWorkReportDate->Text;
		  break;
	 }
	  edWorkflowTitle->Text=seEmpName->Text+L"的"+ ReportStr;
	  mmWorkflowContent->Lines->Add(edWorkflowTitle->Text);
	  mmWorkflowContent->Lines->Add(L"提交时间:"+Now().FormatString("YYYY-MM-DD HH:MM:SS"));
   }
}
//---------------------------------------------------------------------------
