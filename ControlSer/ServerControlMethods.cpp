...
bool TServerControlMethods::StartServer(String ServerName)
{
  ...
  ServerControlModule->NotifyWriteLog(L"启动服务:"+ServerName);
  return bok==0;
}
//----------------------------------------------------------------------------
bool TServerControlMethods::StopServer(String ServerName)
{
  ...
  ServerControlModule->UpdateServerList(ServerName);
  ServerControlModule->NotifyWriteLog(L"停止服务:"+ServerName);
  return bok==0;
}
//----------------------------------------------------------------------------
	  ...
	  if(sNode&&(sNode->GetValue("ServiceStatus")->Value()=="-1"))  //未安装
		{
			...
			__finally
			{
			 ...
			 ServerControlModule->UpdateServerList(ServerName);
		   ServerControlModule->NotifyWriteLog(L"注册服务:"+ServerName);
	  }
	 return retvalue==0;
}
//----------------------------------------------------------------------------
bool TServerControlMethods::UnregServer(String ServerName)
{
	...
	ServerControlModule->UpdateServerList(ServerName);
	 ServerControlModule->NotifyWriteLog(L"卸载服务:"+ServerName);
	return retvalue==0;
}
//----------------------------------------------------------------------------
bool TServerControlMethods::RebootComputer()
{
   ...
   if (!ExitWindowsEx(EWX_REBOOT | EWX_FORCE,
      ...
      return FALSE;
   ServerControlModule->NotifyWriteLog(L"重启机器");
...
bool TServerControlMethods::ShutdownComputer()
{
   ...
   if (!ExitWindowsEx(EWX_SHUTDOWN | EWX_FORCE,
               ...
               SHTDN_REASON_FLAG_PLANNED))
	  return FALSE;
   ServerControlModule->NotifyWriteLog(L"关闭电脑");
