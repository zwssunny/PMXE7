...
begin
 ...
 { //考虑先生成，有可能用户后续才加入项目权限
  if WorkIns <> nil and WorkIns.FlowMnPrjID>''  then
    begin  //如果没有项目权限，则不创建该条任务
       for c :=  AAssignUsers.Count - 1 to 0 do
       begin
         if AAssignUsers.Objects[c] is TWorkflowUser then  //如果是用户
           begin
               AUser := TWorkflowUser(AAssignUsers.Objects[c]);
               if AUser.NeedPrjRight then  //需要过滤项目权限用户
               begin //如果该用户没有项目权限，则删除该条用户。
    ...
    begin
      ...
      try
        ...
        if AAssignUsers.Objects[c] is TWorkflowUser then    {部门主管也转成员工处理}
        begin
          AUser := TWorkflowUser(AAssignUsers.Objects[c]);
        ...
        begin
          ...
          raise EWorkflowException.Create('内部错误，创建任务实例时关联的操作者转化错误');
        ...
        TI.FlowMnDtlRemark := WorkIns.FlowMnSubject+#13#10+TI.TaskDef.Description;  //zws20140510,业务主题+任务描述

        TI.Status := TaskDef.InitialStatus;

       //UserID := '';
        //TI.Comments := TI.TaskDef.Description;   {这是让操作者输入意见的}
        if WorkIns <> nil then
        begin
...
begin
    ...
    begin
      ...
      Connect;
      try
       {先转到正确ftp目录,还没处理,每个流程实例保存一个目录,这样可以防止覆盖 }
...
begin
    ...
    begin
      ...
      Connect;
      try
       {先转到正确ftp目录,然后把内容存到本地硬盘，然后上传,还没处理。
        每个流程实例保存一个目录,这样可以防止覆盖 }
        if (Length(AFtpInfo.Root) > 0) and (AFtpInfo.Root[Length(AFtpInfo.Root)] <> '/') then
          Destpath :=AFtpInfo.Root+'/'+AFtpInfo.SpaceName+'/'+AFtpInfo.FtpDirectory+'/'+AttachmentItem.WorkKey
        else
           DestPath := AFtpInfo.Root+AFtpInfo.SpaceName+'/'+AFtpInfo.FtpDirectory+'/'+AttachmentItem.WorkKey;
        //增加处理转到目标子目录，需要创建子目录功能。
          ChangeDir('/');
          MakeDir(DestPath);
          ChangeDir(DestPath);
       if AttachmentItem.DirtyContent = True then      {包含在里面}
       begin
       //设置临时目录文件
        SetLength(Buffer, MAX_PATH);
        SetLength(Buffer, GetTempPath(MAX_PATH, PChar(Buffer)));
...
begin
   ...
   AFtpInfo.SpaceName := FFtpInformation.SpaceName;{存放父节点，一般是账套目录}
   AFtpInfo.FtpDirectory := FFtpInformation.FtpDirectory; {该业务的目录}
