		...
		try
		{
			...
			for(int i=0;i<ImageIDList->Count;i++)
			{
				...
				if(!FileExists(mFileName))
				{
					...
					csGetData("/Systems/Common/GetImageByGUID",Params);
...
{
	TfrmLoanCalculation *p = new TfrmLoanCalculation(this,ClientBroker);
