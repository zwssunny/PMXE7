...
uses Dialogs,Messages, {$IFDEF LINUX} WinUtils, {$ENDIF} Windows, SysUtils, Classes,
  Variants, rtlconsts,Graphics, Menus, Controls, Forms, StdCtrls, Mask;
...
type
  ...
  TFillEvent = procedure (Sender: TObject; RowStart, RowEnd: Integer) of object;
  {End Add}

  TBYCustomGrid = class;

  TBYInplaceEdit = class(TCustomMaskEdit)
  private
    FGrid: TBYCustomGrid;
    FClickTime: Longint;
    FIsKeyDown: Boolean;
    procedure InternalMove(const Loc: TRect; Redraw: Boolean);
    procedure SetGrid(Value: TBYCustomGrid);
  ...
  protected
    ...
    procedure MyTextOnChange(Sender :TObject);virtual;
    procedure WndProc(var Message: TMessage); override;
    property  Grid: TBYCustomGrid read FGrid;
  public
    OldText: String;
...
published
  ...
  { TBYCustomGrid is an abstract base class that can be used to implement
    ...
    Protected members, for implementors of TBYCustomGrid descendents
      DesignOptionBoost
  ...
  TBYGridDrawState = set of (gdSelected, gdFocused, gdFixed);
  ...
  TDrawCellEvent = procedure (Sender: TObject; ACol, ARow: Longint;
    Rect: TRect; State: TBYGridDrawState) of object;
  TEditControlChange = procedure (Sender: TObject; ACol, ARow: Longint;Text: String) of object;
  TBeforeMovedEvent = procedure (Sender: TObject; FromIndex, ToIndex: Longint;var CanMove: Boolean) of object;

  TBYCustomGrid = class(TCustomControl)
  private
    FAnchor: TGridCoord;
    ...
    FHitTest: TPoint;
    FInplaceEdit: TBYInplaceEdit;
    FInplaceCol, FInplaceRow: Longint;
    FColOffset: Integer;
    FDefaultDrawing: Boolean;
    FEditorMode: Boolean;
    FOnEditControlChange:TEditControlChange;
    FOnBeforeColumnMoved: TBeforeMovedEvent;
    FOnBeforeRowMoved: TBeforeMovedEvent;
    ...
    procedure SetAutoCal(Value: Boolean);
  ...
  protected
    ...
    VirtualView: Boolean;
    procedure ReCalculate(ACol, ARow: Integer);virtual;
    ...
    function CreateEditor: TBYInplaceEdit; virtual;
    procedure CreateParams(var Params: TCreateParams); override;
    procedure KeyDown(var Key: Word; Shift: TShiftState); override;
    procedure KeyPress(var Key: Char); override;
    ...
    procedure DrawCell(ACol, ARow: Longint; ARect: TRect;
      AState: TBYGridDrawState); virtual; abstract;
    ...
    property FixedRowHeight: Integer read FFixedRowHeight write SetFixedRowHeight default 20;
    ...
    property InplaceEditor: TBYInplaceEdit read FInplaceEdit;
    ...
    property HasFooter :Boolean read FHasFooter write SetHasFooter default False;
    property FootHeight :Integer read FFootHeight write SetFootHeight ;
    property AutoCal :Boolean read FAutoCal write SetAutoCal default false;
    property OnEditControlChange: TEditControlChange read FOnEditControlChange write FOnEditControlChange;
    property OnBeforeColumnMoved: TBeforeMovedEvent read FOnBeforeColumnMoved write FOnBeforeColumnMoved;
    property OnBeforeRowMoved: TBeforeMovedEvent read FOnBeforeRowMoved write FOnBeforeRowMoved;
  public
    constructor Create(AOwner: TComponent); override;
  ...
  TBYDrawGrid = class(TBYCustomGrid)
  private
  ...
  protected
    ...
    procedure DrawCell(ACol, ARow: Longint; ARect: TRect;
      AState: TBYGridDrawState); override;
  ...
  published
    ...
    property OnBeforeColumnMoved;
    property OnBeforeRowMoved;
    property OnContextPopup;
    property OnDblClick;
    property OnDragDrop;
    property OnDragOver;
    property OnDrawCell: TDrawCellEvent read FOnDrawCell write FOnDrawCell;
    property OnEditControlChange;
    property OnEndDock;
    property OnEndDrag;
    property OnEnter;
    property OnExit;
  ...
  { TBYStringGrid adds to TBYDrawGrid the ability to save a string and associated
  ...
  TBYStringGridStrings = class(TStrings)
  private
    FGrid: TBYStringGrid;
    FIndex: Integer;
  ...
  public
    constructor Create(AGrid: TBYStringGrid; AIndex: Longint);
  ...
  TBYStringGrid = class(TBYDrawGrid)
  private
    FData: Pointer;
    FRows: Pointer;
    FCols: Pointer;
//    FNeedsUpdating: Boolean;
    ...
    {Start Add by lu, 2002-06-20}
    function GetRecNo(Index: Integer): Integer;
    function GetFilledRow(Index: Integer): Boolean;
    function GetFilledGroup(Index: Integer): Boolean;
    function GetData(Index: Integer): string;
    function GetDoubleData(ACol, ARow: Integer): Double;
    procedure SetRecNo(Index, ARecNo: Integer);
    procedure SetFilledRow(Index: Integer; AValue: Boolean);
    procedure SetFilledGroup(Index: Integer; AValue: Boolean);
    procedure SetData(Index:Integer; AValue: string);
    procedure SetDoubleData(ACol, ARow: Integer; AValue: Double);
    {End Add}
  protected
    function EnsureColRow(Index: Integer; IsCol: Boolean): TBYStringGridStrings;
  ...
  public
    ...
    {Start Add by lu, 2002-06-20}
    property DoubleData[ACol, ARow: Integer]: Double read GetDoubleData write SetDoubleData;
    property RecNo[Index: Integer]: Integer read GetRecNo write SetRecNo;
    property FilledRow[Index: Integer]: Boolean read GetFilledRow write SetFilledRow;
    property FilledGroup[Index: Integer]: Boolean read GetFilledGroup write SetFilledGroup;
...
//    procedure DelRow(Index: Integer);
    {End Add}
  published
    property OnFill : TFillEvent read FOnFill write FOnFill;
  end;
implementation

uses Math, Consts {$ifdef D6_AND_LATER},RTLConsts{$endif};
...
begin
  if Amount <> 0 then
  begin
    if Amount<100 then Amount := 100; //预分配一万
  ...
  //  begin
     // if Extents <> nil then
       // Extents := HeapReAlloc(GetProcessHeap,$00000008,Extents,NewSize * SizeOf(Integer))
     // else
     //   Extents := HeapAlloc(GetProcessHeap,$00000008,NewSize * SizeOf(Integer));
   // end
   // else
   //   Extents :=nil;
...
constructor TBYInplaceEdit.Create(AOwner: TComponent);
begin
...
procedure TBYInplaceEdit.CreateParams(var Params: TCreateParams);
begin
  inherited CreateParams(Params);
  Params.Style := Params.Style or ES_MULTILINE;
end;

procedure TBYInplaceEdit.SetGrid(Value: TBYCustomGrid);
begin
  FGrid := Value;
end;

procedure TBYInplaceEdit.CMShowingChanged(var Message: TMessage);
begin
  { Ignore showing using the Visible property }
end;

procedure TBYInplaceEdit.WMGetDlgCode(var Message: TWMGetDlgCode);
begin
  inherited;
...
procedure TBYInplaceEdit.WMPaste(var Message);
begin
  OldText:=Text;
  if not EditCanModify then Exit;
  inherited;
 if (OldText<>Text) and Assigned(FGrid) and Assigned(FGrid.OnEditControlChange) then
     begin
     FGrid.OnEditControlChange(FGrid,FGrid.Col,FGrid.Row,Text);
     OldText:=Text;
     end;
end;

procedure TBYInplaceEdit.WMClear(var Message);
begin
  OldText:=Text;
  if not EditCanModify then Exit;
  inherited;
...
procedure TBYInplaceEdit.WMCut(var Message);
begin
  OldText:=Text;
  if not EditCanModify then Exit;
  inherited;
...
procedure TBYInplaceEdit.WMUndo(var Message);
begin
  OldText:=Text;
  if not EditCanModify then Exit;
  inherited;
...
procedure TBYInplaceEdit.DblClick;
begin
  Grid.DblClick;
end;

function TBYInplaceEdit.DoMouseWheel(Shift: TShiftState; WheelDelta: Integer;
  MousePos: TPoint): Boolean;
begin
  Result := Grid.DoMouseWheel(Shift, WheelDelta, MousePos);
end;

function TBYInplaceEdit.EditCanModify: Boolean;
begin
  Result := Grid.CanEditModify;
end;

procedure TBYInplaceEdit.KeyDown(var Key: Word; Shift: TShiftState);
...
procedure TBYInplaceEdit.KeyPress(var Key: Char);
var
...
procedure TBYInplaceEdit.KeyUp(var Key: Word; Shift: TShiftState);
begin
    FIsKeyDown :=False;
// if ((OldText<>Text) or (Key in[8,46,48..57,96..105])) and Assigned(FGrid) and Assigned(FGrid.OnEditControlChange) then
 if (OldText<>Text) and Assigned(FGrid) and Assigned(FGrid.OnEditControlChange) then
     begin //(Key=VK_BACK)or( Key=VK_DELETE )
     FGrid.OnEditControlChange(FGrid,FGrid.Col,FGrid.Row,Text);
     OldText:=Text;
     end;
...
procedure TBYInplaceEdit.WndProc(var Message: TMessage);
begin
  case Message.Msg of
    WM_SETFOCUS:
      begin
...
procedure TBYInplaceEdit.Deselect;
begin
  SendMessage(Handle, EM_SETSEL, $7FFFFFFF, Longint($FFFFFFFF));
end;

procedure TBYInplaceEdit.Invalidate;
var
  Cur: TRect;
begin
...
function TBYInplaceEdit.PosEqual(const Rect: TRect): Boolean;
var
  Cur: TRect;
begin
...
procedure TBYInplaceEdit.InternalMove(const Loc: TRect; Redraw: Boolean);
begin
...
procedure TBYInplaceEdit.BoundsChanged;
var
  R: TRect;
begin
...
procedure TBYInplaceEdit.MyTextOnChange(Sender :TObject);
begin
  if(Assigned(FGrid) and Assigned(FGrid.OnEditControlChange)) then
        FGrid.OnEditControlChange(FGrid,FGrid.Col,FGrid.Row,Text);
end;
procedure TBYInplaceEdit.UpdateLoc(const Loc: TRect);
begin
  InternalMove(Loc, False);
end;

function TBYInplaceEdit.Visible: Boolean;
begin
  Result := IsWindowVisible(Handle);
end;

procedure TBYInplaceEdit.Move(const Loc: TRect);
begin
  InternalMove(Loc, True);
end;

procedure TBYInplaceEdit.SetFocus;
begin
  if IsWindowVisible(Handle) then
    Windows.SetFocus(Handle);
end;

procedure TBYInplaceEdit.UpdateContents;
begin
  Text := '';
...
constructor TBYCustomGrid.Create(AOwner: TComponent);
const
...
destructor TBYCustomGrid.Destroy;
begin
  FInplaceEdit.Free;
  inherited Destroy;
  FreeMem(FColWidths);
  FreeMem(FRowHeights);
  FreeMem(FTabStops);
  //HeapFree(GetProcessHeap,HEAP_NO_SERIALIZE ,FColWidths);
  //HeapFree(GetProcessHeap,HEAP_NO_SERIALIZE ,FRowHeights);
  //HeapFree(GetProcessHeap,HEAP_NO_SERIALIZE ,FTabStops);
end;

procedure TBYCustomGrid.AdjustSize(Index, Amount: Longint; Rows: Boolean);
var
  NewCur: TGridCoord;
...
function TBYCustomGrid.BoxRect(ALeft, ATop, ARight, ABottom: Longint): TRect;
var
  GridRect: TGridRect;
begin
  GridRect.Left := ALeft;
...
procedure TBYCustomGrid.DoExit;
begin
  inherited DoExit;
...
function TBYCustomGrid.CellRect(ACol, ARow: Longint): TRect;
begin
  Result := BoxRect(ACol, ARow, ACol, ARow);
end;

function TBYCustomGrid.CanEditAcceptKey(Key: Char): Boolean;
begin
  Result := True;
end;

function TBYCustomGrid.CanGridAcceptKey(Key: Word; Shift: TShiftState): Boolean;
begin
  Result := True;
end;

function TBYCustomGrid.CanEditModify: Boolean;
begin
  Result := FCanEditModify;
end;

function TBYCustomGrid.CanEditShow: Boolean;
begin
...
function TBYCustomGrid.IsActiveControl: Boolean;
var
  H: Hwnd;
...
function TBYCustomGrid.GetEditMask(ACol, ARow: Longint): string;
begin
  Result := '';
end;

function TBYCustomGrid.GetEditText(ACol, ARow: Longint): string;
begin
  Result := '';
end;

procedure TBYCustomGrid.SetEditText(ACol, ARow: Longint; const Value: string);
begin
end;

function TBYCustomGrid.GetEditLimit: Integer;
begin
  Result := 0;
end;

procedure TBYCustomGrid.HideEditor;
begin
  FEditorMode := False;
  HideEdit;
end;

procedure TBYCustomGrid.ShowEditor;
begin
  FEditorMode := True;
  UpdateEdit;
end;

procedure TBYCustomGrid.ShowEditorChar(Ch: Char);
begin
  ShowEditor;
...
procedure TBYCustomGrid.InvalidateEditor;
begin
  FInplaceCol := -1;
  FInplaceRow := -1;
  UpdateEdit;
...
procedure TBYCustomGrid.ReadColWidths(Reader: TReader);
var
  I: Integer;
begin
  with Reader do
...
procedure TBYCustomGrid.ReadRowHeights(Reader: TReader);
var
  I: Integer;
begin
  with Reader do
...
procedure TBYCustomGrid.WriteColWidths(Writer: TWriter);
var
  I: Integer;
begin
  with Writer do
...
procedure TBYCustomGrid.WriteRowHeights(Writer: TWriter);
var
  I: Integer;
begin
  with Writer do
...
procedure TBYCustomGrid.DefineProperties(Filer: TFiler);

  function DoColWidths: Boolean;
  begin
    if Filer.Ancestor <> nil then
      Result := not CompareExtents(TBYCustomGrid(Filer.Ancestor).FColWidths, FColWidths)
    else
  ...
  begin
    if Filer.Ancestor <> nil then
      Result := not CompareExtents(TBYCustomGrid(Filer.Ancestor).FRowHeights, FRowHeights)
    else
...
procedure TBYCustomGrid.MoveColumn(FromIndex, ToIndex: Longint);
var
  Rect: TGridRect;
begin
...
procedure TBYCustomGrid.ColumnMoved(FromIndex, ToIndex: Longint);
begin
end;

procedure TBYCustomGrid.MoveRow(FromIndex, ToIndex: Longint);
begin
...
procedure TBYCustomGrid.RowMoved(FromIndex, ToIndex: Longint);
begin
end;

function TBYCustomGrid.MouseCoord(X, Y: Integer): TGridCoord;
var
...
procedure TBYCustomGrid.MoveColRow(ACol, ARow: Longint; MoveAnchor,
  Show: Boolean);
begin
  MoveCurrent(ACol, ARow, MoveAnchor, Show);
end;

function TBYCustomGrid.SelectCell(ACol, ARow: Longint): Boolean;
begin
  Result := True;
end;

procedure TBYCustomGrid.SizeChanged(OldColCount, OldRowCount: Longint);
begin
end;

function TBYCustomGrid.Sizing(X, Y: Integer): Boolean;
var
  DrawInfo: TGridDrawInfo;
  State: TGridState;
  Index: Longint;
...
procedure TBYCustomGrid.TopLeftChanged;
begin
...
var
  ...
  procedure DrawCells(ACol, ARow: Longint; StartX, StartY, StopX, StopY: Integer;
    Color: TColor; IncludeDrawState: TBYGridDrawState);
  var
    CurCol, CurRow: Longint;
    AWhere, Where, TempRect: TRect;
    DrawState: TBYGridDrawState;
    Focused: Boolean;
  begin
    CurRow := ARow;
    Where.Top := StartY;
...
function TBYCustomGrid.CalcCoordFromPoint(X, Y: Integer;
  const DrawInfo: TGridDrawInfo): TGridCoord;
...
procedure TBYCustomGrid.CalcDrawInfo(var DrawInfo: TGridDrawInfo);
begin
  CalcDrawInfoXY(DrawInfo, ClientWidth, ClientHeight);
end;

procedure TBYCustomGrid.CalcDrawInfoXY(var DrawInfo: TGridDrawInfo;
  UseWidth, UseHeight: Integer);
...
begin
  ...
  if FHasFooter then  //zws2003-12-23
   CalcAxis(DrawInfo.Vert, UseHeight-FFootHeight)
  else
   CalcAxis(DrawInfo.Vert, UseHeight);
   CalcAxis(DrawInfo.Horz, UseWidth);
end;

procedure TBYCustomGrid.CalcFixedInfo(var DrawInfo: TGridDrawInfo);
...
function TBYCustomGrid.CalcMaxTopLeft(const Coord: TGridCoord;
  const DrawInfo: TGridDrawInfo): TGridCoord;
...
procedure TBYCustomGrid.CalcSizingState(X, Y: Integer; var State: TGridState;
  var Index: Longint; var SizingPos, SizingOfs: Integer;
  ...
  procedure CalcAxisState(const AxisInfo: TGridAxisDrawInfo; Pos: Integer;
    NewState: TGridState);
  var
    I, Line, Back, Range,FromIndex: Integer;
  begin
    if UseRightToLeftAlignment then
      Pos := ClientWidth - Pos;
    with AxisInfo do
    begin
      if (Pos>FixedBoundary) then
          Line := FixedBoundary
      else
          Line := 0;
     ...
     // for I := FirstGridCell to GridCellCount - 1 do
      if (Pos>FixedBoundary) then
          FromIndex := FirstGridCell
      else
          FromIndex := 0;

      for I := FromIndex to GridCellCount - 1 do
      begin
...
begin
  ...
  if [goColSizing, goRowSizing] * EffectiveOptions <> [] then
    ...
    begin
      ...
      if (X>0) and (goColSizing in EffectiveOptions) then
      begin
...
procedure TBYCustomGrid.ChangeGridOrientation(RightToLeftOrientation: Boolean);
var
  Org: TPoint;
  Ext: TPoint;
begin
...
procedure TBYCustomGrid.ChangeSize(NewColCount, NewRowCount: Longint);
var
...
procedure TBYCustomGrid.ClampInView(const Coord: TGridCoord);
var
  DrawInfo: TGridDrawInfo;
...
begin
  ...
  begin
    if (X > Horz.LastFullVisibleCell) or
      (Y > Vert.LastFullVisibleCell) or ((not (goRowSelect in Options)) and (X < LeftCol)) or (Y < TopRow) then
    begin
      OldTopLeft := FTopLeft;
...
procedure TBYCustomGrid.DrawSizingLine(const DrawInfo: TGridDrawInfo);
var
  OldPen: TPen;
begin
  OldPen := TPen.Create;
...
procedure TBYCustomGrid.DrawMove;
var
  OldPen: TPen;
  Pos: Integer;
  R: TRect;
...
procedure TBYCustomGrid.FocusCell(ACol, ARow: Longint; MoveAnchor: Boolean);
begin
...
procedure TBYCustomGrid.GridRectToScreenRect(GridRect: TGridRect;
  var ScreenRect: TRect; IncludeLine: Boolean);
...
procedure TBYCustomGrid.Initialize;
begin
...
procedure TBYCustomGrid.InvalidateCell(ACol, ARow: Longint);
var
  Rect: TGridRect;
begin
  Rect.Top := ARow;
...
procedure TBYCustomGrid.InvalidateCol(ACol: Longint);
var
  Rect: TGridRect;
begin
  if not HandleAllocated then Exit;
...
procedure TBYCustomGrid.InvalidateRow(ARow: Longint);
var
  Rect: TGridRect;
begin
  if not HandleAllocated then Exit;
...
procedure TBYCustomGrid.ReCalculate;
begin
end;
procedure TBYCustomGrid.SetAutoCal(Value: Boolean);
begin
   if FAutoCal <> Value then
   begin
    FAutoCal :=Value;
    if FAutoCal and FHasFooter then
    begin
    ReCalculate(-1,-1);
    InvalidateGrid;
    end;
...
procedure TBYCustomGrid.InvalidateGrid;
begin
  Invalidate;
end;

procedure TBYCustomGrid.InvalidateRect(ARect: TGridRect);
var
  InvalidRect: TRect;
begin
  if not HandleAllocated then Exit;
...
procedure TBYCustomGrid.ModifyScrollBar(ScrollBar, ScrollCode, Pos: Cardinal;
  UseRightToLeft: Boolean);
var
...
procedure TBYCustomGrid.MoveAdjust(var CellPos: Longint; FromIndex, ToIndex: Longint);
var
  Min, Max: Longint;
begin
...
procedure TBYCustomGrid.MoveAnchor(const NewAnchor: TGridCoord);
var
  OldSel: TGridRect;
begin
...
procedure TBYCustomGrid.MoveCurrent(ACol, ARow: Longint; MoveAnchor,
  Show: Boolean);
var
  OldSel: TGridRect;
...
procedure TBYCustomGrid.MoveTopLeft(ALeft, ATop: Longint);
var
  OldTopLeft: TGridCoord;
begin
...
procedure TBYCustomGrid.ResizeCol(Index: Longint; OldSize, NewSize: Integer);
begin
  InvalidateGrid;
end;

procedure TBYCustomGrid.ResizeRow(Index: Longint; OldSize, NewSize: Integer);
begin
  InvalidateGrid;
end;

procedure TBYCustomGrid.SelectionMoved(const OldSel: TGridRect);
var
...
procedure TBYCustomGrid.ScrollDataInfo(DX, DY: Integer;
...
procedure TBYCustomGrid.ScrollData(DX, DY: Integer);
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
procedure TBYCustomGrid.TopLeftMoved(const OldTopLeft: TGridCoord);
...
procedure TBYCustomGrid.UpdateScrollPos;
var
  DrawInfo: TGridDrawInfo;
  MaxTopLeft: TGridCoord;
  GridSpace, ColWidth: Integer;
...
procedure TBYCustomGrid.UpdateScrollRange;
var
  ...
  var
    Min, Max,c,allColWidths: Integer;
  begin
    Result := False;
      ...
      if Code=SB_HORZ then
       begin
        ...
        for c:=0 to ColCount-1 do
          Inc(allColWidths,ColWidths[c]);
        Result:=allColWidths > ClientWidth;
        if not Result then  FTopLeft.X := FixedCols;
        ShowScrollBar(Handle,Code,Result);
       end
      else
      begin
  ...
  var
    Range: Integer;
  begin
    if ( OldScrollBars in [ssHorizontal, ssBoth] ) and ScrollBarVisible(SB_HORZ) then
      if ColCount = 1 then
      begin
...
function TBYCustomGrid.CreateEditor: TBYInplaceEdit;
begin
  Result := TBYInplaceEdit.Create(Self);
end;

procedure TBYCustomGrid.CreateParams(var Params: TCreateParams);
begin
  inherited CreateParams(Params);
  with Params do
  begin
...
procedure TBYCustomGrid.KeyDown(var Key: Word; Shift: TShiftState);
var
...
procedure TBYCustomGrid.KeyPress(var Key: Char);
begin
...
procedure TBYCustomGrid.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  CellHit: TGridCoord;
  DrawInfo: TGridDrawInfo;
...
begin
  ...
  begin
    ...
    begin
      ...
      (CellHit.Y < FixedRows) and (CellHit.X >= 0) then
    begin
...
procedure TBYCustomGrid.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  DrawInfo: TGridDrawInfo;
  CellHit: TGridCoord;
begin
...
procedure TBYCustomGrid.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  DrawInfo: TGridDrawInfo;
  NewSize: Integer;
    ...
    begin
      //修改固定列也可以拖动    by hp 2006-12-11
        ...
        if (FSizingIndex>FixedCols) then
           ...
           FromIndex := 0;
        for I := FromIndex to FSizingIndex - 1 do
        Inc(Result, GetExtent(I) + EffectiveLineWidth);
      if ((FSizingIndex<=FixedCols) and (FixedBoundary+FSizingPos - Result-ColWidths[FSizingIndex]>=GridBoundary) ) then
         Result := 0
      else
    ...
    case FGridState of
      ...
      gsColMoving:
        begin
          ...
          begin
           CanMove:=True;
           if Assigned(FOnBeforeColumnMoved) then FOnBeforeColumnMoved(Self,FMoveIndex,FMovePos,CanMove);
            if CanMove then
      ...
      gsRowMoving:
        begin
          ...
          begin
            CanMove:=True;
           if Assigned(FOnBeforeRowMoved) then FOnBeforeRowMoved(Self,FMoveIndex,FMovePos,CanMove);
           if CanMove then
...
procedure TBYCustomGrid.MoveAndScroll(Mouse, CellHit: Integer;
...
function TBYCustomGrid.GetColWidths(Index: Longint): Integer;
begin
...
function TBYCustomGrid.GetRowHeights(Index: Longint): Integer;
begin
...
function TBYCustomGrid.GetGridWidth: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
function TBYCustomGrid.GetGridHeight: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
function TBYCustomGrid.GetSelection: TGridRect;
begin
  Result := GridRect(FCurrent, FAnchor);
end;

function TBYCustomGrid.GetTabStops(Index: Longint): Boolean;
begin
  if FTabStops = nil then Result := True
  else Result := Boolean(PIntArray(FTabStops)^[Index + 1]);
end;

function TBYCustomGrid.GetVisibleColCount: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
function TBYCustomGrid.GetVisibleRowCount: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
procedure TBYCustomGrid.SetBorderStyle(Value: TBorderStyle);
begin
...
procedure TBYCustomGrid.SetCol(Value: Longint);
begin
  if Col <> Value then FocusCell(Value, Row, True);
end;

procedure TBYCustomGrid.SetColCount(Value: Longint);
var
  ColCountCapacity: Integer;
  I: Integer;
begin
  if FColCount <> Value then
  begin
    if Value < 1 then Value := 1;
//    if Value < FixedCols then FixedCols := Value;
    if Value <= FixedCols then FixedCols := Value - 1;//zws2003-11-17
    ...
    if FColWidths <> nil then
    begin
      ColCountCapacity := PIntArray(FColWidths)^[0];
      for I:=RowCount+1 to ColCountCapacity do
        PIntArray(FColWidths)^[I] := DefaultColWidth;
    end;
  end;
end;

procedure TBYCustomGrid.SetColWidths(Index: Longint; Value: Integer);
begin
  if FColWidths = nil then
    UpdateExtents(FColWidths, ColCount, DefaultColWidth);
...
procedure TBYCustomGrid.SetDefaultColWidth(Value: Integer);
var
  I: Integer;
  ColCountCapacity: Integer;
begin
  ...
  begin
    ...
    ColCountCapacity := PIntArray(FColWidths)^[0];
    for I:=ColCount to ColCountCapacity do   //zws20090506 ,fixd Bug
      PIntArray(FColWidths)^[I] := Value;
  end;
...
procedure TBYCustomGrid.SetDefaultRowHeight(Value: Integer);
var I: Integer;
  RowCountCapacity: Integer;
begin
  if FRowHeights <> nil then
  begin
    UpdateExtents(FRowHeights, 0, 0);
    RowCountCapacity := PIntArray(FRowHeights)^[0];
    for I:=RowCount to RowCountCapacity do   //zws20090506 ,fixd Bug
      PIntArray(FRowHeights)^[I] := Value;
  end;
  FDefaultRowHeight := Value;
  for I:= 0 to FixedRows-1 do
     RowHeights[I]:=FFixedRowHeight;
  RowHeightsChanged;
  InvalidateGrid;
end;

procedure TBYCustomGrid.SetFixedRowHeight(Value: Integer);
var
  I: Integer;
begin
...
procedure TBYCustomGrid.SetFixedColor(Value: TColor);
begin
...
procedure TBYCustomGrid.SetFixedCols(Value: Integer);
begin
...
procedure TBYCustomGrid.SetHasFooter(Value: Boolean);
begin
   if FHasFooter <> Value then
   begin
    FHasFooter :=Value;
    Initialize;
    UpdateScrollPos;
...
procedure TBYCustomGrid.SetFootHeight(Value: Integer);
begin
   if FFootHeight <> Value then
   begin
    FFootHeight :=Value;
    if FHasFooter then
      begin
...
procedure TBYCustomGrid.SetFixedRows(Value: Integer);
var I: Integer;
begin
...
procedure TBYCustomGrid.SetEditorMode(Value: Boolean);
begin
  if not Value then
    HideEditor
  else
...
procedure TBYCustomGrid.SetGridLineWidth(Value: Integer);
begin
...
procedure TBYCustomGrid.SetLeftCol(Value: Longint);
begin
  if FTopLeft.X <> Value then MoveTopLeft(Value, TopRow);
end;

procedure TBYCustomGrid.SetOptions(Value: TGridOptions);
begin
...
procedure TBYCustomGrid.SetRow(Value: Longint);
begin
  if Row <> Value then FocusCell(Col, Value, True);
end;

procedure TBYCustomGrid.SetRowCount(Value: Longint);
var
  RowCountCapacity: Integer;
  I: Integer;  
begin
  ...
  begin
    ...
    begin
      RowCountCapacity := PIntArray(FRowHeights)^[0];
      for I:=RowCount+1 to RowCountCapacity do
        PIntArray(FRowHeights)^[I] := DefaultRowHeight;
    end;
  end;
end;

procedure TBYCustomGrid.SetRowHeights(Index: Longint; Value: Integer);
begin
  if FRowHeights = nil then
    UpdateExtents(FRowHeights, RowCount, DefaultRowHeight);
...
procedure TBYCustomGrid.SetScrollBars(Value: TScrollStyle);
begin
...
procedure TBYCustomGrid.SetSelection(Value: TGridRect);
var
  OldSel: TGridRect;
begin
  OldSel := Selection;
...
procedure TBYCustomGrid.SetTabStops(Index: Longint; Value: Boolean);
begin
  if FTabStops = nil then
    UpdateExtents(FTabStops, ColCount, Integer(True));
  if Index >= ColCount then InvalidOp(SIndexOutOfRange);
...
procedure TBYCustomGrid.SetTopRow(Value: Longint);
begin
  if FTopLeft.Y <> Value then MoveTopLeft(LeftCol, Value);
end;

procedure TBYCustomGrid.HideEdit;
begin
  if FInplaceEdit <> nil then
    try
      UpdateText;
...
procedure TBYCustomGrid.UpdateEdit;
...
procedure TBYCustomGrid.WMCommand(var Message: TWMCommand);
begin
  with Message do
  begin
...
procedure TBYCustomGrid.WMGetDlgCode(var Msg: TWMGetDlgCode);
begin
...
procedure TBYCustomGrid.WMKillFocus(var Msg: TWMKillFocus);
begin
  inherited;
  InvalidateRect(Selection);
...
procedure TBYCustomGrid.WMLButtonDown(var Message: TMessage);
begin
  inherited;
  if FInplaceEdit <> nil then FInplaceEdit.FClickTime := GetMessageTime;
end;

procedure TBYCustomGrid.WMNCHitTest(var Msg: TWMNCHitTest);
begin
  DefaultHandler(Msg);
  FHitTest := ScreenToClient(SmallPointToPoint(Msg.Pos));
end;

procedure TBYCustomGrid.WMSetCursor(var Msg: TWMSetCursor);
var
  DrawInfo: TGridDrawInfo;
  State: TGridState;
  Index: Longint;
...
procedure TBYCustomGrid.WMSetFocus(var Msg: TWMSetFocus);
begin
  inherited;
...
procedure TBYCustomGrid.WMSize(var Msg: TWMSize);
begin
  inherited;
  UpdateScrollRange;
//  if UseRightToLeftAlignment then    //zws2003-12-23
   Invalidate;
end;

procedure TBYCustomGrid.WMVScroll(var Msg: TWMVScroll);
begin
  ModifyScrollBar(SB_VERT, Msg.ScrollCode, Msg.Pos, True);
end;

procedure TBYCustomGrid.WMHScroll(var Msg: TWMHScroll);
begin
  ModifyScrollBar(SB_HORZ, Msg.ScrollCode, Msg.Pos, True);
end;

procedure TBYCustomGrid.CancelMode;
var
  DrawInfo: TGridDrawInfo;
begin
  try
...
procedure TBYCustomGrid.WMCancelMode(var Msg: TWMCancelMode);
begin
  inherited;
  CancelMode;
end;

procedure TBYCustomGrid.CMCancelMode(var Msg: TMessage);
begin
...
procedure TBYCustomGrid.CMFontChanged(var Message: TMessage);
begin
  if FInplaceEdit <> nil then FInplaceEdit.Font := Font;
  inherited;
end;

procedure TBYCustomGrid.CMCtl3DChanged(var Message: TMessage);
begin
  inherited;
  RecreateWnd;
end;

procedure TBYCustomGrid.CMDesignHitTest(var Msg: TCMDesignHitTest);
begin
  Msg.Result := Longint(BOOL(Sizing(Msg.Pos.X, Msg.Pos.Y)));
end;

procedure TBYCustomGrid.CMWantSpecialKey(var Msg: TCMWantSpecialKey);
begin
  inherited;
  if (goEditing in Options) and (Char(Msg.CharCode) = #13) then Msg.Result := 1;
end;

procedure TBYCustomGrid.TimedScroll(Direction: TGridScrollDirection);
var
...
procedure TBYCustomGrid.WMTimer(var Msg: TWMTimer);
var
  Point: TPoint;
  DrawInfo: TGridDrawInfo;
...
procedure TBYCustomGrid.ColWidthsChanged;
begin
  UpdateScrollRange;
  UpdateEdit;
end;

procedure TBYCustomGrid.RowHeightsChanged;
begin
  UpdateScrollRange;
  UpdateEdit;
end;

procedure TBYCustomGrid.DeleteColumn(ACol: Longint);
begin
  MoveColumn(ACol, ColCount-1);
  ColCount := ColCount - 1;
end;

procedure TBYCustomGrid.DeleteRow(ARow: Longint);
begin
  MoveRow(ARow, RowCount - 1);
  RowCount := RowCount - 1;
end;

procedure TBYCustomGrid.UpdateDesigner;
var
  ParentForm: TCustomForm;
begin
...
function TBYCustomGrid.DoMouseWheelDown(Shift: TShiftState; MousePos: TPoint): Boolean;
begin
...
function TBYCustomGrid.DoMouseWheelUp(Shift: TShiftState; MousePos: TPoint): Boolean;
begin
...
function TBYCustomGrid.CheckColumnDrag(var Origin,
  Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TBYCustomGrid.CheckRowDrag(var Origin,
  Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TBYCustomGrid.BeginColumnDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TBYCustomGrid.BeginRowDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TBYCustomGrid.EndColumnDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TBYCustomGrid.EndRowDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

procedure TBYCustomGrid.CMShowingChanged(var Message: TMessage);
begin
  inherited;
...
function TBYDrawGrid.CellRect(ACol, ARow: Longint): TRect;
begin
  Result := inherited CellRect(ACol, ARow);
end;

procedure TBYDrawGrid.MouseToCell(X, Y: Integer; var ACol, ARow: Longint);
var
  Coord: TGridCoord;
begin
...
procedure TBYDrawGrid.ColumnMoved(FromIndex, ToIndex: Longint);
begin
  if Assigned(FOnColumnMoved) then FOnColumnMoved(Self, FromIndex, ToIndex);
end;

function TBYDrawGrid.GetEditMask(ACol, ARow: Longint): string;
begin
  Result := '';
  if Assigned(FOnGetEditMask) then FOnGetEditMask(Self, ACol, ARow, Result);
end;

function TBYDrawGrid.GetEditText(ACol, ARow: Longint): string;
begin
  Result := '';
  if Assigned(FOnGetEditText) then FOnGetEditText(Self, ACol, ARow, Result);
end;

procedure TBYDrawGrid.RowMoved(FromIndex, ToIndex: Longint);
begin
  if Assigned(FOnRowMoved) then FOnRowMoved(Self, FromIndex, ToIndex);
end;

function TBYDrawGrid.SelectCell(ACol, ARow: Longint): Boolean;
begin
  Result := True;
  if Assigned(FOnSelectCell) then FOnSelectCell(Self, ACol, ARow, Result);
end;

procedure TBYDrawGrid.SetEditText(ACol, ARow: Longint; const Value: string);
begin
  if Assigned(FOnSetEditText) then FOnSetEditText(Self, ACol, ARow, Value);
end;

procedure TBYDrawGrid.DrawCell(ACol, ARow: Longint; ARect: TRect;
  AState: TBYGridDrawState);
var
  Hold: Integer;
begin
...
procedure TBYDrawGrid.TopLeftChanged;
begin
...
function NewStrItem(const AString: string; AObject: TObject; DValue: Double): PStrItem;
begin
  New(Result);
  Result^.FObject := AObject;
  Result^.FString := AString;
  Result^.DoubleData := DValue;
end;
...
{ Sparse array classes for TBYStringGrid }
...
{ TStringSparseList class }
  ...
  public
    ...
    FilledGroup: boolean;
    Data: String;
    function GetDoubleData(Index: Integer): Double;
    procedure SetDoubleData(Index: Integer; AValue: Double);
    {End Add By Lu}
    constructor Create(Quantum: TSPAQuantum);
    destructor  Destroy; override;
...
begin
  Result := secDir;
{  if Result <> nil then
    Result := HeapReAlloc(GetProcessHeap,$00000008,Result,newSlots * SizeOf(Pointer))
  else
    Result := HeapAlloc(GetProcessHeap,$00000008,newSlots * SizeOf(Pointer));
  if Result = nil then
    Raise Exception.Create('Cannot allocate memory from heap'); }
...
begin
  ...
  if secDir <> nil then    //zws 2003-10-27
    FreeMem(secDir, slotsInDir * SizeOf(Pointer));
    //HeapFree(GetProcessHeap,0,secDir);
end;
...
begin
  ...
  if p = nil then d := 0 else d := p^.DoubleData;
  if (S = '') and (obj = nil) then   { Nothing left to store }
    FList[Index] := nil
  else
    FList[Index] := NewStrItem(S, obj, d);
...
begin
  ...
  if p <> nil then
    ...
    FList[Index] := NewStrItem('', AObject, 0);
  Changed
end;
...
begin
  FList.Insert(Index, NewStrItem(S, nil, 0));
  Changed
end;
...
{Start Add By lu, 2002-06-20}
function TStringSparseList.GetDoubleData(Index: Integer): Double;
var
  p: PStrItem;
begin
  p := PStrItem(FList[Index]);
...
procedure TStringSparseList.SetDoubleData(Index: Integer; AValue: Double);
var
  p: PStrItem;
begin
  p := PStrItem(FList[Index]);
  if p <> nil then
     ...
     FList[Index] := NewStrItem('', Nil, AValue);
end;
{End Add By Lu}

{ TBYStringGridStrings }

{ AIndex < 0 is a column (for column -AIndex - 1)
  AIndex > 0 is a row (for row AIndex - 1)
  AIndex = 0 denotes an empty row or column }

constructor TBYStringGridStrings.Create(AGrid: TBYStringGrid; AIndex: Longint);
begin
  inherited Create;
  FGrid := AGrid;
  FIndex := AIndex;
...
procedure TBYStringGridStrings.Assign(Source: TPersistent);
var
  I, Max: Integer;
begin
...
procedure TBYStringGridStrings.CalcXY(Index: Integer; var X, Y: Integer);
begin
  if FIndex = 0 then
  begin
    X := -1; Y := -1;
...
function TBYStringGridStrings.Add(const S: string): Integer;
var
  I: Integer;
begin
...
procedure TBYStringGridStrings.Clear;
var
...
procedure TBYStringGridStrings.Delete(Index: Integer);
begin
  InvalidOp(sInvalidStringGridOp);
end;

function TBYStringGridStrings.Get(Index: Integer): string;
var
  X, Y: Integer;
begin
  CalcXY(Index, X, Y);
...
function TBYStringGridStrings.GetCount: Integer;
begin
...
function TBYStringGridStrings.GetObject(Index: Integer): TObject;
var
  X, Y: Integer;
begin
  CalcXY(Index, X, Y);
...
procedure TBYStringGridStrings.Insert(Index: Integer; const S: string);
begin
  InvalidOp(sInvalidStringGridOp);
end;

procedure TBYStringGridStrings.Put(Index: Integer; const S: string);
var
  X, Y: Integer;
begin
  CalcXY(Index, X, Y);
...
procedure TBYStringGridStrings.PutObject(Index: Integer; AObject: TObject);
var
  X, Y: Integer;
begin
  CalcXY(Index, X, Y);
...
procedure TBYStringGridStrings.SetUpdateState(Updating: Boolean);
begin
...
constructor TBYStringGrid.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  Initialize;
end;
...
{$IFEND}
destructor TBYStringGrid.Destroy;
...
procedure TBYStringGrid.ColumnMoved(FromIndex, ToIndex: Longint);
...
procedure TBYStringGrid.RowMoved(FromIndex, ToIndex: Longint);
begin
...
function TBYStringGrid.GetEditText(ACol, ARow: Longint): string;
begin
  Result := Cells[ACol, ARow];
  if Assigned(FOnGetEditText) then FOnGetEditText(Self, ACol, ARow, Result);
end;

procedure TBYStringGrid.SetEditText(ACol, ARow: Longint; const Value: string);
begin
  DisableEditUpdate;
  try
...
procedure TBYStringGrid.DrawCell(ACol, ARow: Longint; ARect: TRect;
  AState: TBYGridDrawState);
var
   TextFlags : integer;
begin
...
procedure TBYStringGrid.DisableEditUpdate;
begin
  Inc(FEditUpdate);
end;

procedure TBYStringGrid.EnableEditUpdate;
begin
  Dec(FEditUpdate);
end;

procedure TBYStringGrid.Initialize;
var
  quantum: TSPAQuantum;
begin
  if FCols = nil then
...
procedure TBYStringGrid.SetUpdateState(Updating: Boolean);
begin
  FUpdating := Updating;
...
procedure TBYStringGrid.Update(ACol, ARow: Integer);
begin
...
function  TBYStringGrid.EnsureColRow(Index: Integer; IsCol: Boolean):
  TBYStringGridStrings;
var
  RCIndex: Integer;
  PList: ^TSparseList;
begin
  ...
  Result := TBYStringGridStrings(PList^[Index]);
  if Result = nil then
  begin
    if IsCol then RCIndex := -Index - 1 else RCIndex := Index + 1;
    Result := TBYStringGridStrings.Create(Self, RCIndex);
    PList^[Index] := Result;
  end;
end;

function  TBYStringGrid.EnsureDataRow(ARow: Integer): Pointer;
var
  quantum: TSPAQuantum;
begin
...
function TBYStringGrid.GetCells(ACol, ARow: Integer): string;
var
...
function TBYStringGrid.GetCols(Index: Integer): TStrings;
begin
  Result := EnsureColRow(Index, True);
end;

function TBYStringGrid.GetObjects(ACol, ARow: Integer): TObject;
var
  ssl: TStringSparseList;
begin
  ssl := TStringSparseList(TSparseList(FData)[ARow]);
...
function TBYStringGrid.GetRows(Index: Integer): TStrings;
begin
  Result := EnsureColRow(Index, False);
end;

procedure TBYStringGrid.SetCells(ACol, ARow: Integer; const Value: string);
begin
   TBYStringGridStrings(EnsureDataRow(ARow))[ACol] := Value;
  EnsureColRow(ACol, True);
  EnsureColRow(ARow, False);
  if AutoCal and HasFooter and not (csLoading in ComponentState) then
     ReCalculate(ACol, ARow);           //zws20050615
  Update(ACol, ARow);
end;
procedure TBYStringGrid.SetCols(Index: Integer; Value: TStrings);
begin
  EnsureColRow(Index, True).Assign(Value);
end;

procedure TBYStringGrid.SetObjects(ACol, ARow: Integer; Value: TObject);
begin
  TBYStringGridStrings(EnsureDataRow(ARow)).Objects[ACol] := Value;
  EnsureColRow(ACol, True);
  EnsureColRow(ARow, False);
  if not FUpdating then
  begin
...
procedure TBYStringGrid.SetRows(Index: Integer; Value: TStrings);
begin
  EnsureColRow(Index, False).Assign(Value);
end;

procedure TBYStringGrid.BeginUpdate();
begin
   SetUpdateState(true);
end;

procedure TBYStringGrid.EndUpdate();
begin
...
{Start Add by lu, 2002-06-20}
function TBYStringGrid.GetRecNo(Index: Integer): Integer;
begin
   Result := TStringSparseList(TSparseList(FData)[Index]).RecNo;
end;

function TBYStringGrid.GetFilledRow(Index: Integer): Boolean;
begin
   Result := TStringSparseList(TSparseList(FData)[Index]).FilledRow;
end;

function TBYStringGrid.GetFilledGroup(Index: Integer): Boolean;
begin
   Result := TStringSparseList(TSparseList(FData)[Index]).FilledGroup;
end;

function TBYStringGrid.GetData(Index: Integer): string;
begin
   Result := TStringSparseList(TSparseList(FData)[Index]).Data;
end;

function TBYStringGrid.GetDoubleData(ACol, ARow: Integer): Double;
begin
   Result := TStringSparseList(TSparseList(FData)[ARow]).GetDoubleData(ACol);
end;

procedure TBYStringGrid.SetRecNo(Index, ARecNo: Integer);
begin
   TStringSparseList(TSparseList(FData)[Index]).RecNo := ARecNo;
end;

procedure TBYStringGrid.SetFilledRow(Index: Integer; AValue: Boolean);
begin
   TStringSparseList(TSparseList(FData)[Index]).FilledRow := AValue;
end;

procedure TBYStringGrid.SetFilledGroup(Index: Integer; AValue: Boolean);
begin
   TStringSparseList(TSparseList(FData)[Index]).FilledGroup := AValue;
end;

procedure TBYStringGrid.SetData(Index:Integer; AValue: string);
begin
   TStringSparseList(TSparseList(FData)[Index]).Data := AValue;
end;

procedure TBYStringGrid.SetDoubleData(ACol, ARow: Integer; AValue: Double);
begin
   TStringSparseList(TSparseList(FData)[ARow]).SetDoubleData(ACol, AValue);
end;

procedure TBYStringGrid.InsertRow(Index: Integer);
var
  quantum: TSPAQuantum;
  aRow : TStringSparseList;
begin
    if ColCount > 512 then quantum := SPALarge else quantum := SPASmall;
    aRow := TStringSparseList.Create(quantum);
    TSparseList(FData).Insert(Index, aRow);
    EnsureDataRow(Index);
    inherited SetRowCount(RowCount + 1);
end;

procedure TBYStringGrid.DeleteRow(ARow: Longint);
var
    temp : TStringSparseList;
begin
    temp := TStringSparseList(TSparseList(FData)[ARow]);
    TSparseList(FData)[ARow] := nil;
    if temp <> nil then
       temp.Destroy;
//    TSparseList(FData).Delete(ARow);
    inherited DeleteRow(ARow);
end;

procedure TBYStringGrid.SetRowCount(Value: Longint);
var
   I ,c: Integer;
   OldRowCount : Integer;
   temp : TStringSparseList;
...
begin
   ...
   inherited;
   if OldRowCount = Value then
      Exit;
   if OldRowCount < Value then
   begin
     for I := OldRowCount to Value do
       EnsureDataRow(I);
   end
   else
   begin
     ...
     try
       AutoCal :=False;
       for I := OldRowCount downto Value do
       begin
          for c:= 0 to ColCount-1 do  //zws2003-12-05
             Cells[c,I]:='';
        if TStringSparseList(TSparseList(FData)[I-1]) <> nil then
         begin
           temp := TStringSparseList(TSparseList(FData)[i]);
           if temp <> nil then
              temp.Clear();
           TSparseList(FData)[i] := nil;
           temp.Destroy;  //zws2003-12-05
         end;
       end;
    finally
    AutoCal :=OldAutoCal;
