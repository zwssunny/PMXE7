...
void __fastcall TMsgStream::DecClientMsg()
{
	...
	try
	{
		...
		if (QIDField != NULL) // zws20101104兼容底版本。
		...
		/* ms->Clear();
		 ...
		 DecompressStream(ms, FMasterDataSet); // 解压
		...
		/* ms->Clear();
		 ...
		 DecompressStream(ms, FDetailDataSet); // 解压
...
void __fastcall TMsgStream::BuildClientMsg()
{
	...
	try
	{
		...
		FMasterDataSet->Position = 0;
		/* ms->Clear();
		 CompressStream(FMasterDataSet, ms); // 压缩
		 ms->Position = 0;
		...
		FDetailDataSet->Position = 0;
		/* ms->Clear();
		 CompressStream(FDetailDataSet, ms); // 压缩
		 ms->Position = 0;
