...
begin
  ...
  begin
    ...
    frAssignedUser.SaveEditorsInTask(FLastTask);
    FLastTask := nil;
  end
end;
...
procedure TfmTaskEditor.btCopyClick(Sender: TObject);
var
  NewTask: TTaskDefinition;
  SelectTask: TTaskDefinition;
  NewName: string;
  i: integer;
  NewItem: TListItem;
...
begin
 if lvTasks.SelCount > 0  then
  ...
  NewTask := FTasks.Add;
  SelectItem:= lvTasks.Selected;
  if SelectItem<>nil then
  begin
    SelectTask:= TaskDefFromItem(SelectItem);
    NewTask.Assign(SelectTask);
  end;
  ...
  if NewTask.StatusList.Count =0 then
  begin
    NewTask.StatusList.Add('执行中');
    NewTask.StatusList.Add('*已完成');
  end;
