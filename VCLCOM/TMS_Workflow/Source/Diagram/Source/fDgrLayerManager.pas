...
begin
  if MessageDlg('删除图层 "' + SelectedLayer.LayerName + '"?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
  begin
    SelectedLayer.Free;
    LoadLayers;
  end;
...
begin
  if MessageDlg('删除图层 "' + SelectedLayer.LayerName + '" 及其所有图形吗?', mtWarning, [mbYes, mbNo], 0) = mrYes then
  begin
