...
type
  ...
  public
    ...
    function DefineMethod(AName: string; AArgCount: integer; AResultDataType: TatTypeKind; AResultClass: TClass;
      AProc: TMachineProc; AIsClassMethod: boolean = false; ADefArgCount: integer = 0;
      AParameterHints: string = ''):TatMethod;
    ...
    property ScriptBaseObjectClass: TScriptBaseObjectClass read FScriptBaseObjectClass write SetScriptBaseObjectClass;
  ...
  TScriptBaseObject = class(TPersistent)
  private
    FDestroying: boolean;
  ...
  private
    FScripter                   : TatCustomScripter;
    FParser                     : TatSyntaxParser; // referência ?sintaxe da linguagem
    ...
    FExpandedDebugInfo          : boolean; // indica que o campo vDebugInfo j?contem linha e coluna do programa-fonte
    ...
    procedure GetRowColFromInst(Inst: pSimplifiedCode; var Row,
      Col: integer);
    function DefineProp(AName: string;
      ADataType: TatTypeKind; AGetter, ASetter: TMachineProc;
      APropertyClass: TClass = nil; AIsClassProp: boolean = false;
  ...
  private
    ...
    FMainBegin                  : pSimplifiedCode; // aponta para a instrução de preparação do bloco principal (quando script j?inicializado)
    ...
    procedure ExecuteDebugScript(AScript: TatScript; AScripterWatch: TatScripterWatch = nil; ADebugWatch: TatDebugWatch = nil);
...
//    FIsClass: boolean;
  ...
  private
    ...
    FArgIndex               : integer; // indica que a variável ?um argumento de subrotina
  ...
  public
    ...
    function SetProc(AName: string; ACode: pointer; AArgCount: integer; AResultDataType: TatTypeKind; AProc: TGenericProc;
      AResultClass: TClass; AIsClassMethod: boolean = false; ADefArgCount: integer = 0;
      AParameterHints: string = ''): TatMethod;
    ...
    ///   AParameterHints - Optionally you can pass textual info used by scripter to provide code parameter hints. The string must be
    ///     in pascal format, types and default values can be ommited. Examples:
  ...
  private
    ...
    procedure SetPropExtra(AGetter, ASetter: TGenericProc; AValue: Variant);
  protected
  ...
  public
    constructor Create(AScript: TatScript);
    function Add(AClassIndex, AElementIndex, AElementClassIndex: integer;
  ...
  private
    ...
    procedure FillMatchingItems(AClass: TatClass;
      AName: string; AList: TStrings);
    procedure FillAllMatchingItems(AClass: TatClass;
      AName: string; AList: TStrings);
    function FindResultClass(AClass: TatClass; AName: string): TatClass;
    function FindAnyResultClass(AClass: TatClass; AName: string): TatClass;
    class function IsClassDefaultInstance(AClass: TatClass): boolean;
    procedure AddItemToList(AList: TStrings; AClassName, AName: string;
      AValue: TObject);
  ...
  public
    ...
    /// Parameters:
    ///   AInputText - The current source code of the script (Script.SourceCode property is not used)
    ///   AInputPos - The current cursor position in the input text in which parameter hint will be displayed
    ///   AToken - the returned token used for parameter hint. This is usually the qualified name of the
    ///     method for which parameter hints will be displayed (example, 'MyForm.Show')
    ///   AParIndex - The index of parameter to be displayed (0 is first parameter, 1 second, etc.
    /// Return Value:
    ///   Function returns true if there is a valid parameter hint to be displayed at this position. False otherwise.
    function GetParameterHintInfo(AInputText: string; AInputPos: integer;
      var AToken: string; var AParIndex: integer): boolean;

    /// Description:
    ///   Returns the parameter hint to be displayed given an specified token
    /// Parameters:
    ///   AToken - the token used to display the parameter hint. Usually this token is previously retrieved
    ///     using GetParameterHintInfo
    /// Return Value:
    ///   The parameter hint to be displayed in text format
    function GetParameterHint(AToken: string): string;
    ...
    /// Parameters:
    ///   AInputText - The current source code of the script (Script.SourceCode property is not used)
    ///   AInputPos - The current cursor position in the input text in which code completion will be displayed
    ///   ADefaultIndex - The suggested index of the completion list to be selected. This is only calculated if Context property is set.
    /// Return value:
    ...
    ///   The types of objects that can be in the Objects property are:
    ...
    function GetCodeCompletion(AInputText: string; AInputPos: integer; var DefaultIndex: integer): TStringList;
    ...
    function CreateCodeCompletionList(AInputText: string; AInputPos: integer; var DefaultIndex: integer): TStringList;
...
begin
  ...
  begin
    { monta um conjunto através dos itens passados no vetor de variants
      os elementos são tratados como bits e o conjunto ?formado pela união
      desses bits }
    result := 0;
...
function CreateCodeCompletionList(AScript: TatScript; AInputText:string; AInputPos:integer; var AExprPos, AExprLen: integer): TStringList;
var
  D: integer;
begin
...
begin
   ...
   { cria a estrutura que receber?as informações das classes do scripter }
   ...
   {Define create for TComponent to avoid problems after declaring TScriptForm.Create. If we don't do that,
    ...
    Create}
   DefineClass(TComponent).DefineMethod('Create', 1, tkClass, TComponent, ComponentCreateProc, true, 0, 'AOwner:TComponent');
   DefineClass(TScriptForm).DefineMethod('Create', 1, tkClass, TScriptForm, ScriptFormCreateProc, true, 0, 'AOwner:TComponent');
   DefineClass(TScriptBaseObject).DefineMethod('Create', 0, tkClass, TScriptBaseObject, ScriptBaseObjectCreateProc, true);
...
procedure TatCustomScripter.DefineProperties(Filer: TFiler);
  ...
  begin
     ...
     if Filer.Ancestor <> nil then
       ...
       { se não tem ancestral então s?grava caso o conteúdo seja não nulo }
       Result := Compiled and SaveCompiledCode;
  end;
...
begin
   ...
   { l?o tamanho do stream, como cabeçalho }
...
function TatCustomScripter.FindFileInSearchPath(const CompName, SrcName: string): string;
  ...
  begin
    if CompareText(SCurDir, Copy(S, 1, Length(SCurDir))) = 0 then
      ...
      {$IFDEF DELPHI6_LVL}
      result := ExcludeTrailingPathDelimiter(ExtractFilePath(Application.ExeName)) + Copy(S, Length(SAppDir)+1, Length(S))
      {$ELSE}
      result := ExcludeTrailingBackslash(ExtractFilePath(Application.ExeName)) + Copy(S, Length(SAppDir)+1, Length(S))
      {$ENDIF}
    else
      result := S;
  end;
...
function TatCustomScripter.DefineMethod(AName: string; AArgCount: integer;
  ...
  AIsClassMethod: boolean = false; ADefArgCount: integer = 0;
  AParameterHints: string = ''): TatMethod;
begin
   ...
   begin
      ...
      for c:=0 to ASourceScripter.FClasses.Count-1 do
         ...
         begin
            ...
            // Ancestor ?ajustado posteriormente
         end;
      ...
      { a instância 0 sempre existe e ?interna, então a copia de modo especial }
...
begin
  ...
  AMachine.ReturnOutputArg(integer(AObject));
end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              scriptMethod := result.DefineMethod(
                method.Name,
                Length(Pars),
                typeKind,
                regClass,
...
begin
   ...
   { se a classe tiver um ancestral j?conhecido então herda dele todas
...
begin
   with SetProp(AName,tkVariant,nil,nil,nil) do
     SetPropExtra(GenericProc(GetVariantPropProc), nil, AValue);
end;
...
begin
   try
      with SetProp(AName,tkVariant,nil,nil,nil) do
        SetPropExtra(
          GenericProc(GetVarVariablePropProc),
          GenericProc(SetVarVariablePropProc),
          integer(PVariant(@AVar)));
   except
...
begin
   try
      with SetProp(AName,tkVariant,nil,nil,nil) do
        SetPropExtra(
          GenericProc(GetExtVariablePropProc),
          GenericProc(SetExtVariablePropProc),
          integer(PExtended(@AVar)));
   except
      DefinitionError('Variable is unassigned or not a variant');
   end;
end;
...
begin
   try
      with SetProp(AName,tkVariant,nil,nil,nil) do
        SetPropExtra(
          GenericProc(GetDblVariablePropProc),
          GenericProc(SetDblVariablePropProc),
          integer(PDouble(@AVar)));
   except
      DefinitionError('Variable is unassigned or not a variant');
   end;
end;
...
begin
   try
      with SetProp(AName,tkVariant,nil,nil,nil) do
        SetPropExtra(
          GenericProc(GetStrVariablePropProc),
          GenericProc(SetStrVariablePropProc),
          integer(PString(@AVar)));
   except
      DefinitionError('Variable is unassigned or not a variant');
   end;
end;
...
begin
   try
      with SetProp(AName,tkVariant,nil,nil,nil) do
        SetPropExtra(
         GenericProc(GetWStrVariablePropProc),
         GenericProc(SetWStrVariablePropProc),
         integer(PWideString(@AVar)));
   except
      DefinitionError('Variable is unassigned or not a variant');
   end;
end;
...
begin
   try
      with SetProp(AName,tkVariant,nil,nil,nil) do
        SetPropExtra(
          GenericProc(GetIntVariablePropProc),
          GenericProc(SetIntVariablePropProc),
          integer(PInteger(@AVar)));
   except
      DefinitionError('Variable is unassigned or not a variant');
   end;
end;
...
begin
   try
      with SetProp(AName,tkVariant,nil,nil,nil) do
        SetPropExtra(
          GenericProc(GetBoolVariablePropProc),
          GenericProc(SetBoolVariablePropProc),
          integer(PBoolean(@AVar)));
   except
      DefinitionError('Variable is unassigned or not a variant');
   end;
end;
...
begin
   with SetProp(AName,tkClass,nil,nil,AObject.ClassType) do
     SetPropExtra(
       GenericProc(GetDelphiObjectPropProc),
       nil,
       integer(AObject));
end;
...
var i:integer;
begin
   { se o método j?existe, modifica-o senão adiciona-o }
      ...
      for i:=0 to FDescendants.Count-1 do
         ...
         {$ENDIF}
           TatClass(FDescendants[i]).SetProc(
              FName, FCode, FArgCount, FResultDataType, FProc, FResultClass, FIsClassMethod, FDefArgCount, AParameterHints);
   end;
end;
...
function TatClass.DefineMethod(AName: string; AArgCount: integer; AResultDataType: TatTypeKind; AResultClass: TClass; AProc: TMachineProc;
  ...
  AParameterHints: string = ''):TatMethod;
begin
   result := SetProc(AName, nil, AArgCount, AResultDataType, TGenericProc(AProc), AResultClass, AIsClassMethod, ADefArgCount, AParameterHints);
end;
     ...
     tkClass:
       if Assigned(FPropertyClass) then
          if FScripter.FDeferObjectResolution then
             { A resolução do acesso ?classe dever?ocorrer efetivamente em runtime }
...
procedure TatProperty.SetPropExtra(AGetter, ASetter: TGenericProc;
  AValue: Variant);
var
  AClass: TatClass;
  i: integer;
...
begin
  ...
  { alguns tipos não padrão estão sendo tratados como padrão porque seu tamanho ?o mesmo de outros padrão (ex: byte, word, double, ...) }
...
begin
  ...
  { se o tipo ?uma classe então armazena uma referência para a classe do scripter }
...
begin
   ...
   FIntInstance := FDefInstances.Add(AObject, '', 'TatInternalObject' + IntToStr(integer(AObject)));
end;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                if ImplicitClassRef.ElementIsMethod then
                  ImplicitElementClass := TatMethod(ImplicitElement).atClass
                else
                  ImplicitElementClass := TatProperty(ImplicitElement).atClass;
                ImplicitClassRef.ElementClassIndex := ImplicitElementClass.Index;
                ImplicitClassRef.ElementClassNameSize := Length(ImplicitElementClass.Name);
                ...
                AStream.WriteBuffer(PChar(ImplicitElementClass.Name)^, ImplicitClassRef.ElementClassNameSize * SizeOf(Char));
              end;
            end;
          end;
     ...
     try
        ...
        begin
           ...
           implicitClasses.Add(
              ...
              ImplicitClassRef.ElementClassIndex,
...
begin
   ...
   { encontra o especificador se j?existir, senão o cria }
   LabelSpec:=FindLabelSpec(name);
   if LabelSpec=nil then LabelSpec:=NewLabelSpec(name);
   if LabelSpec^.Address>-1 then
   begin
      { se o rótulo j?tiver sido definido, não registra a referência }
...
function TatScript.DefineProp(AName: string; ADataType: TatTypeKind; AGetter,
  ASetter: TMachineProc; APropertyClass: TClass; AIsClassProp: boolean;
  AIndexCount: integer): TatProperty;
begin
...
begin
   ...
   { A resolução das referências ser?feita para a instrução subseqüente ao rótulo }
...
begin
   ...
   begin
      with LabelSpec^ do
         ...
         begin
            { se for um script do debugger então d?uma segunda chance para resolver a referência:
              verifica se o endereço se refere a uma rotina com endereço j?definido }
  ...
  with AVar, FLastInstruction^ do
    if not Global then {*** variáveis globais não suportam otimização por enquanto ***}
      case OpCode of
        ...
        { Como a expressão não ?uma simples constante ou variável então gera uma
   ...
   case FLastInstruction^.OpCode of
      inPushInteger:
         ...
         begin
            ...
            vInteger:=RegisterReference(AJumpLabel); // aqui ser?armazenado o endereço do desvio condicional
      ...
      { Como a expressão não ?uma simples constante nem variável então gera uma
...
begin
   ...
   { se a variável j?existe apenas determina seu índice }
...
begin
  {$IFDEF TRIAL}
  if (Now > 41300) or (Now < 40600) then
  begin
...
begin
   ...
   begin
      { verifica se o método compile j?foi chamado, senão chama-o automaticamente }
      if not FCompiled and RequiresCompilation then
        Compile;

//    { verifica se o endereçamento absoluto j?foi resolvido, senão resolve-o automaticamente }
      ...
      begin
         ...
         begin
            ...
            { *** este procedimento ser?desnecessário quando o parser j?tiver esta
...
begin
  AMachine.ReturnOutputArg(integer(FOriginalObject));
end;
...
begin
   ...
   begin
      ...
      { se a variável PrepareInstruction estiver com algum valor então um script
        j?est?em execução, portanto esta chamada ?re-entrante (não devendo
        interferir com a execução j?disparada) }
...
begin
  ...
  if (AObject is TScriptForm) and (TScriptForm(AObject).RunScript <> nil) then
    result := TScriptForm(AObject).RunScript.ScriptInfo.ScriptClassName;
end;
...
begin
   with TVarData(FProcStack[FCurrentInputArgBase+AIndex]) do
      if VType and varByRef = 0 then
         result := VarToInt64(FProcStack[FCurrentInputArgBase + AIndex])
      else
         result := VarToInt64(PVariant(VPointer)^);
end;
...
begin
   { executa instruções do script at?encontrar o
...
begin
   ...
   begin
      { se a instrução j?for um break point então a remove, senão a define como break }
...
begin
   ...
   { devolvendo 1 se o primeiro ?diferente do segundo , e 0 em caso contrário }
...
begin
   { compara dois valores de topo do FStack }
   { devolvendo 1 se o primeiro ?maior ou igula ao segundo , e 0 em caso contrário }
   FProcStack[FStackTop-2]:=(FProcStack[FStackTop-2]>=FProcStack[FStackTop-1]);
   dec(FStackTop);
end;
...
begin
   { compara dois valores de topo do FStack }
   { devolvendo 1 se o primeiro ?menor ou igual ao segundo , e 0 em caso contrário }
   FProcStack[FStackTop-2]:=(FProcStack[FStackTop-2]<=FProcStack[FStackTop-1]);
   dec(FStackTop);
end;
...
begin
   { compara dois valores de topo do FStack }
   { devolvendo 1 se o primeiro ?igual ao segundo , e 0 em caso contrário }
   FProcStack[FStackTop-2]:=(FProcStack[FStackTop-2]=FProcStack[FStackTop-1]);
   dec(FStackTop);
end;
...
begin
   { compara dois valores de topo do FStack }
   { devolvendo 1 se o primeiro ?maior que o segundo , e 0 em caso contrário }
   FProcStack[FStackTop-2]:=(FProcStack[FStackTop-2]>FProcStack[FStackTop-1]);
   dec(FStackTop);
end;
...
begin
   { compara dois valores de topo do FStack }
   { devolvendo 1 se o primeiro ?menor que o segundo , e 0 em caso contrário }
   FProcStack[FStackTop-2]:=(FProcStack[FStackTop-2]<FProcStack[FStackTop-1]);
   dec(FStackTop);
end;
...
// para gravar dados em uma variável que ?por referência
// with TVarData(FProcStack[FStackBase+FCurrentInstruction^.vInteger]) do
//    if VType and varByRef=0 then
//       FProcStack[FStackBase+FCurrentInstruction^.vInteger]:=FProcStack[FStackTop-1]
//    else
...
begin
   ...
   begin
      ...
      if not VarIsArray(V) and (VarIsType(v, [varString,
        ...
        {$ENDIF}
         varOleStr])) then
         if ADim=vByte then // s?aceita acesso aos caracteres da string no último indice
...
begin
   { armazena o conteúdo da string que est?em vString na variável indexada por vInteger }
   ...
   try
        try
           ...
           begin
              ...
              FLastExceptionHelpContext:=ex.HelpContext;
              FLastExceptionClassName:=ex.ClassName;
              { em caso de exceção num bloco try..finally o stack retornar?a situação
...
begin
   ...
   try
      ...
      { evita que o bloco EXCEPT seja executado, j?que não ocorreu nenhuma exceção }
      ...
      begin
         ...
         FLastExceptionHelpContext:=ex.HelpContext;
         FLastExceptionClassName:=ex.ClassName;
         { em caso de exceção num bloco try..except o stack retornar?a situação
...
begin
  { ajusta o objeto que ser?usado pelo CallProc obrigatoriamente subsequente
...
begin
   ...
   begin
      ...
      begin
         { AQUI A INSTÂNCIA ASSOCIADA AO PROC ?PASSADA POR ÚLTIMO NA PILHA }
      ...
      begin
         { AQUI A INSTÂNCIA ASSOCIADA AO PROC ?PASSADA POR PRIMEIRO NA PILHA }
...
var obj : TObject;
begin
 { testa se o objeto contido no topo da pilha corresponde ou ?derivado da
  ...
  begin
    ...
    { assume que o topo da pilha ?um objeto }
    ...
    { teste se o objeto corresponde ou ?derivado da classe determinada }
...
begin
   ...
   begin
      calling := (vSmallInt = 1);
      { se o objeto corrente não estiver definido tenta encontr?lo no topo da pilha }
      ...
      { localiza a propriedade através de seu nome, na classe da instância
        que est?no topo da pilha. Se a classe ainda não havia sido carregada,
      ...
      begin
         ...
         {$ENDIF}
            loadClass := Scripter.FClasses.CheckLoaded(CurrentObject.ClassType, GetScriptObjectClassName(CurrentObject));
...
begin
  ...
  begin
     ...
     { faz a pesquisa também nos métodos, porque, em runtime,
       eles são tratados como propriedades readonly, neste caso ser?
        ...
        begin
           ...
           if Assigned(property_obj) then
              { se a propriedade puder ser escrita, então escreve o
                conteúdo do nível 2 da pilha na propriedade correspondente ?
                instância que est?no topo da pilha e desempilha dois níveis }
  ...
  begin
     ...
     begin
        { localiza o método através de seu nome, na classe da instância
          que est?no topo da pilha. Se a classe ainda não havia sido carregada,
          então também carrega suas informações através do RTTI }
        method_obj:=Scripter.FClasses.CheckLoaded(CurrentObject.ClassType, GetScriptObjectClassName(CurrentObject)).Methods.MethodByName(vString);
        { verifica se o método realmente existe na classe }
        if Assigned(method_obj) then
           { se o método foi definido então executa-o, eliminando a
             instância correspondente, que est?no top da pilha }
           if Assigned(method_obj.Proc) then
           begin
              { verifica se o número de parâmetros reais est?correto }
                  ...
                  begin
                     ...
                     case TClassReference(integer(TMethod(vProc).Data) and 3) of
                        crGetter : if TMethod(vProc).Data=Pointer(crGetter) then
                                     ...
                                     if FFastSolving or TempFastSolve then
  ...
  begin
    ...
    try
      ...
      try
        ...
        Assert(AScript.FDefInstances.Count > 0, 'CompileDebugScript error: FDefInstances.Count must be > 0');
        ...
        { corrige a intrução de preparação da subrotina porque ela
          redeclarou todas as variáveis j?existentes e mais as novas do script }
      ...
      try
        ...
        begin
          ...
          begin
            if AStackPos <> -1 then
            begin
              AScripterWatch.FLastResult := Self.FProcStack[AStackPos];
          ...
          begin
            if AStackPos <> -1 then
            begin
              ADebugWatch.LastResult := Self.FProcStack[AStackPos];
...
begin
  { remove a referência ao script que est?sendo destruído }
  FDebugScript := nil;
end;
...
begin
  ...
  try
    ...
    on ex:Exception do
    begin
      { em caso de falha de compilação apenas sinaliza no watch que ?inválido }
...
begin
  ...
  try
    ...
    on ex:Exception do
    begin
      { em caso de falha de execução apenas sinaliza no watch que ?inválido }
...
begin
  { limpa informações de runtime do watch, deixando apenas a parte de seu
    conteúdo que não ?dependente da execução }
  DebugScript.Clear;
...
begin
  ...
  if AMachine.CurrentObject is TScriptBaseObject then
...
begin
  ...
  try
    ...
    FRunScript.ScriptInfo.Behavior := sbForm;
...
begin
  ...
  {Retrieve initial information from the AMachine}
  if AMachine.Script.Scripter = nil then
    AMachine.RunTimeError('Cannot creat script-based object: Scripter property of machine is not specified.');
  if AMachine.CurrentClass <> nil then
    ABaseScript := AMachine.CurrentClass.FBaseScript
  else
...
procedure TScripterCodeInsight.SetIgnorePairs(
  const APairs: array of TIgnorePair);
var
  c: Integer;
begin
...
class function TScripterCodeInsight.IsClassDefaultInstance(AClass: TatClass): boolean;
var
  c: integer;
begin
  result := False;
  if AClass.Scripter <> nil then
    for c := 0 to AClass.Scripter.DefaultInstances.Count - 1 do
      if AClass.Scripter.DefaultInstances[c].atClass = AClass then
      begin
        result := True;
        exit;
      end;
...
procedure TScripterCodeInsight.AddItemToList(AList: TStrings; AClassName, AName: string; AValue: TObject);
var
  i: integer;
begin
  ...
  for i := 0 to AList.Count - 1 do
    if (AnsiCompareText(AList[i], AName) = 0) and (AList.Objects[i] = AValue) then Exit;

  AList.AddObject(AName, AValue);
end;

function TScripterCodeInsight.FindAnyResultClass(AClass: TatClass; AName: string): TatClass;
var
  c: integer;
  avar: TatVariableInfo;
  ...
  begin
    ...
    begin
      if (avar.DeclaredClass <> nil) and (avar.DeclaredClass.ClassRef <> nil) then
        FScript.Scripter.Classes.TestCheckLoaded(avar.DeclaredClass.ClassRef, result, avar.DeclaredClassName);
      Exit;
    end;
    ...
    begin
      ...
      begin
        FScript.Scripter.Classes.TestCheckLoaded(aroutine.ResultClass.ClassRef, result, aroutine.ResultClassName);
      end;
      Exit;
    end;
...
begin
  ...
  { não est?no final então tenta determinar a classe do objeto corrente }
...
function TScripterCodeInsight.CreateCodeCompletionList(AInputText: string;
...
function TScripterCodeInsight.GetParameterHint(AToken: string): string;
var
  SL: TStringList;
  P1, P2: integer;
begin
  result := '';
  SL := GetCompletionListFromToken(AToken, true, P1 {dummy});
  try
    if SL.Count = 1 then
    begin
      P1 := Pos('(', SL[0]);
...
function TScripterCodeInsight.GetParameterHintInfo(AInputText: string;
  AInputPos: integer; var AToken: string; var AParIndex: integer): boolean;
var
  StackIdx: integer;
  ...
  begin
    ...
    begin
      if TatProperty(obj).DataType <> tkMethod then
        ...
        value := 3; //event
      if IsClassDefaultInstance(TatProperty(obj).atClass) then
        value := value + 16;
    end;
    ...
    begin
      if TatMethod(obj).ResultDataType = tkNone then
        ...
        value := 2;
      if IsClassDefaultInstance(TatMethod(obj).atClass) then
        value := value + 16;
    end;
...
begin
  ...
  result := GetCompletionListFromToken(expr, false, DefaultIndex);
end;
  ...
  try
    ...
    try
      ...
      begin
        ...
        if c = Count - 1 then
        begin
          FillAllMatchingItems(cls, Strings[c], slist);
          if AExactMatch then
        ...
        begin
          ...
          cls := FindAnyResultClass(cls, Strings[c]);
...
function TCodeCompletionEntries.Add(AToken,
  AValue: string): TCodeCompletionEntry;
begin
