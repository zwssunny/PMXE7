...
void __fastcall TClientBroker::SendUserMessage(int type, String FID)
{
   if(FSendUserMessageClient==NULL)
	FSendUserMessageClient=new TSendUserMessageClient(DSConnection->DBXConnection, FInstanceOwner);
  FSendUserMessageClient->SendUserMessage(type,FID);
}
// ---------------------------------------------------------------------------
// 接收界面数据
// ---------------------------------------------------------------------------
...
void __fastcall TClientBroker::ShowForm(int modulecode, String param,bool bShow)
{
	...
	{
	  ShowMessage( String(L"创建窗体错误," + e.Message));
	}
}
// ---------------------------------------------------------------------------
...
//			PostMessage(Application->MainForm->Handle, WM_FORMS_CHANGED, formid, 1);
	...
	{
	ShowMessage(_D("关闭窗体错误,") + e.Message);
	}
}
// ---------------------------------------------------------------------------
...
void __fastcall TClientBroker::LogOut()
{
	try
	{
		...
		if(FSendUserMessageClient)
		{
		    delete FSendUserMessageClient;
			FSendUserMessageClient=NULL;
		}
		if(LoginUserClient)
		{
		  ...
		  if(FEvent->WaitFor(2000)!=TWaitResult::wrSignaled)
		  {
			 DSConnection->Close();
			 delete LoginUserClient;
			 LoginUserClient=NULL;
